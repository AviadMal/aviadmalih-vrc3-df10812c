...
path = os.path.join(f"{os.path.expanduser('~')}","Token.txt")
if os.path.exists("/ElcSoftware/misc/Python36TK/bin/python3"):
    ...
    WEBDRIVER_CHROME = f"/Projects/Firmver/users/chromedriver"
s = requests.session()
...
def add_task(title, description, type, member, project , area, iteration, repo_name,test_name,seed):
    ...
    div_lines = [f"<div>{line}</div>" for line in lines if line.strip()]
    description = "\n".join(div_lines)
...
def get_bug_id(bug_title):
    ...
    try:
        ...
        for workitem_id in workitem_ids:
            ...
            workitem = response.json()
            if(workitem['fields']['System.WorkItemType'] == 'Bug'):
                if(workitem['fields']['System.Title']==bug_title):
                    return  workitem["id"]
